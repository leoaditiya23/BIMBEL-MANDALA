...
<div x-data="{ activeTab: '{{ \Carbon\Carbon::now()->locale('id')->dayName }}' }" class="min-h-screen pb-12">
    ...
    <div x-show="activeTab === '{{ $day }}'" 
        ...
        @forelse($daySchedule as $item)
            @php
                ...
                // Logika Deteksi Jam Live (Asumsi durasi kelas 90 menit)
                $isLive = false;
                ...
                <div class="flex flex-col md:flex-row md:items-center gap-6 p-4">
                    ...
                    <div class="flex items-center gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-8">
                        <div class="text-right hidden lg:block mr-4">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cabang</p>
                            <p class="text-xs font-bold text-slate-600">{{ $item->lokasi_cabang ?? '-' }}</p>
                        </div>
                        
                        {{-- Tombol dengan mode fokus 'session=active' --}}
