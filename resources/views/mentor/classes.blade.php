...
<div x-data="{ 
    ...
    // Fungsi untuk membuka modal nilai/tugas
    openNilaiModal(item) {
        this.selectedClass = JSON.parse(JSON.stringify(item));
    ...
    {{-- MODAL NILAI / PENILAIAN --}}
    <div x-show="modalTugas" class="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" x-cloak x-transition>
        <div @click.away="modalTugas = false" class="bg-white rounded-[40px] shadow-2xl w-full max-w-4xl overflow-hidden border border-white flex flex-col h-auto max-h-[85vh]">
            <div class="p-8 border-b border-slate-100 flex justify-between items-center bg-white">
                <div>
                    <h3 class="text-2xl font-black text-slate-800 leading-tight">Penilaian Siswa</h3>
                    <p class="text-[10px] font-black text-indigo-500 uppercase tracking-widest mt-1" x-text="selectedClass.name + ' (' + (selectedClass.kelas || '-') + ')'"></p>
                </div>
                <button @click="modalTugas = false" class="text-slate-300 hover:text-rose-500 transition-colors"><i class="fas fa-times-circle text-2xl"></i></button>
            </div>
            
            <div class="p-8 overflow-y-auto custom-scrollbar bg-slate-50/50">
                <div class="space-y-4">
                    <template x-for="material in selectedClass.materials" :key="material.id">
                        <div class="bg-white p-6 rounded-[25px] border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
                            <div class="flex items-center gap-5 flex-1">
                                <div class="w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 font-black text-sm" x-text="material.session_number"></div>
                                <div>
                                    <h4 class="font-black text-slate-800 text-sm leading-tight mb-1" x-text="material.title"></h4>
                                    <span class="text-[9px] font-black bg-indigo-100 text-indigo-600 px-2 py-0.5 rounded uppercase tracking-tighter" x-text="'Siswa: ' + selectedClass.student_name"></span>
                                </div>
                            </div>
                            
                            <form action="{{ route('mentor.grades.store') }}" method="POST" class="flex flex-wrap md:flex-nowrap items-center gap-3 w-full md:w-auto">
                                @csrf
                                <input type="hidden" name="program_id" :value="selectedClass.id">
                                <input type="hidden" name="student_id" :value="selectedClass.user_id">
                                <input type="hidden" name="title" :value="'Sesi ' + material.session_number + ': ' + material.title">
                                
                                <input type="number" name="score" placeholder="Nilai" required min="0" max="100" class="w-20 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-black focus:ring-2 focus:ring-indigo-500">
                                <input type="text" name="note" placeholder="Catatan Mentor..." class="flex-1 md:w-48 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold focus:ring-2 focus:ring-indigo-500">
                                <button type="submit" class="bg-indigo-600 text-white px-5 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all shadow-md shadow-indigo-100">Simpan</button>
                            </form>
                        </div>
                    </template>
                </div>
