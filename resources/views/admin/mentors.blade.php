...
<div class="w-full pb-20 relative z-10" x-data="{ 
    ...
    @if ($errors->any())
    <div class="mb-6 p-4 bg-red-500 text-white font-bold rounded-2xl shadow-lg shadow-red-100">
        <ul class="list-disc list-inside text-xs uppercase tracking-tight">
    ...
    <template x-teleport="body">
        <div x-show="showMentorModal" class="fixed inset-0 flex items-center justify-center p-4" style="z-index: 999999999 !important;" x-cloak>
            ...
            <div x-show="showMentorModal" class="relative bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl border-[4px] border-slate-50 overflow-y-auto max-h-[90vh]">
                <div class="mb-5">
                    <h3 class="text-lg font-black text-slate-900 tracking-tight">Tambah Mentor & Akun</h3>
                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Daftarkan profil dan akun login mentor</p>
                </div>
                ...
                <form action="{{ route('admin.mentors.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-3.5">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" required value="{{ old('name') }}" placeholder="Nama lengkap" class="w-full px-4 py-2.5 rounded-lg bg-[#f8fafc] border-2 border-slate-100 outline-none font-bold text-slate-800 text-xs focus:border-blue-500 transition">
                        </div>

                        {{-- Akun Login (Penting!) --}}
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1 px-1">Username</label>
                                <input type="text" name="username" required value="{{ old('username') }}" placeholder="untuk login" class="w-full px-4 py-2.5 rounded-lg bg-[#f8fafc] border-2 border-slate-100 outline-none font-bold text-slate-800 text-xs focus:border-blue-500 transition">
                            </div>
                            <div>
                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1 px-1">Password</label>
                                <input type="password" name="password" required placeholder="********" class="w-full px-4 py-2.5 rounded-lg bg-[#f8fafc] border-2 border-slate-100 outline-none font-bold text-slate-800 text-xs focus:border-blue-500 transition">
                            </div>
                        </div>

                        <div>
                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1 px-1">Email</label>
                            <input type="email" name="email" required value="{{ old('email') }}" placeholder="email@gmail.com" class="w-full px-4 py-2.5 rounded-lg bg-[#f8fafc] border-2 border-slate-100 outline-none font-bold text-slate-800 text-xs focus:border-blue-500 transition">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="text" name="specialist" required value="{{ old('specialist') }}" placeholder="Fisika, dll" class="w-full px-4 py-2.5 rounded-lg bg-[#f8fafc] border-2 border-slate-100 outline-none font-bold text-slate-800 text-xs focus:border-blue-500 transition">
                            </div>
                            <div>
                                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1 px-1">No. WhatsApp</label>
                                <input type="text" name="whatsapp" required value="{{ old('whatsapp') }}" placeholder="0812..." class="w-full px-4 py-2.5 rounded-lg bg-[#f8fafc] border-2 border-slate-100 outline-none font-bold text-slate-800 text-xs focus:border-blue-500 transition">
                            </div>
                        </div>
                        ...
                        <div class="mt-6 flex gap-2.5 pt-1">
                            ...
                            <button type="submit" class="flex-1 py-3 rounded-lg font-black text-white bg-blue-600 shadow-md shadow-blue-100 uppercase text-[9px] tracking-wider hover:bg-blue-700 transition">Simpan & Buat Akun</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <template x-teleport="body">
        <div x-show="showEditModal" class="fixed inset-0 flex items-center justify-center p-4" style="z-index: 999999999 !important;" x-cloak>
            ...
            <div x-show="showEditModal" class="relative bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl border-[4px] border-slate-50 overflow-y-auto max-h-[90vh]">
                <div class="mb-5">
                    <h3 class="text-lg font-black text-slate-900 tracking-tight">Edit Profil Mentor</h3>
                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Perbarui data profil dan keahlian mentor</p>
                </div>
...
<style>
    ...
    input { background-color: #f8fafc !important; color: #0f172a !important; opacity: 1 !important; }
